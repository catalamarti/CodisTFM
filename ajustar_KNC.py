...
for i in range(len(CC)):
	clf = KNeighborsClassifier(n_neighbors=CC[i],algorithm='kd_tree')
	clf.fit(X_t,Y_t)
	Y_p = clf.predict(X_v)
	error= np.zeros(N)
	for ii in range(0,N):
...
clf = KNeighborsClassifier(n_neighbors=np.int_(cc[id]),algorithm='kd_tree')
clf.fit(X_t,Y_t)
Y_p = clf.predict(X_v)
error= np.zeros(N)
for ii in range(0,N):
